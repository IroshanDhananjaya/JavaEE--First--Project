...
function generateOrderID() {
    $.ajax({
        url:"http://localhost:8080/JavaEEPosSystem/order",
        method:"GET",
        success(resp){

            try {
                let lastOID = resp.data[resp.data.length - 1].oId;
...
function loadCustIDs(){
    ...
    $.ajax({
    url:"http://localhost:8080/JavaEEPosSystem/customer?option=GETALL",
        method:"GET",
        success(resp) {
...
$("#cmbCustIDs").click(function () {
    ...
    $.ajax({
        url:"http://localhost:8080/JavaEEPosSystem/customer",
        method:"Get",
        success(resp){
            for (var i of resp.data) {
                if(cus==i.id){
...
function loadAllOrder(){
    ...
    $.ajax({
        ...
        success(resp) {
            console.log(resp.data);
            for (var i of resp.data) {
                var row = `<tr><td>${i.oId}</td><td>${i.cId}</td><td>${i.oDate}</td></tr>`;
...
function loadAllOrderDetails(){
    ...
    $.ajax({
        url:"http://localhost:8080/JavaEEPosSystem/orderDetails",
        method:"GET",
        success(resp) {
            console.log(resp.data);
            for (var i of resp.data) {
                var row = `<tr><td>${i.oId}</td><td>${i.itemCode}</td><td>${i.iPrice}</td><td>${i.oQty}</td><td>${i.total}</td><tr>`;
